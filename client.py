...
async def _send_msg(message: str, host: str, port: int, loop):
    client = Client()
...
async def _receive_msg(host: str, port: int, loop):
    client = Client()
    await client.connect(host=host, port=port, loop=loop)
...
if __name__ == "__main__":
    ...
    if args.send:
        loop.run_until_complete(_send_msg(message, host=HOST, port=PORT, loop=loop))
    if args.receive:
        loop.run_until_complete(_receive_msg(host=HOST, port=PORT, loop=loop))
    loop.close()
